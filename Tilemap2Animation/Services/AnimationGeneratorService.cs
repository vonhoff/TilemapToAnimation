...
public class AnimationGeneratorService : IAnimationGeneratorService
{
    ...
    public int CalculateTotalAnimationDuration(Tileset tileset, int frameDelay)
    {
        ...
        try
        {
            ...
            var animatedTiles =
                tileset.Tiles?.Where(t => t.Animation?.Frames != null && t.Animation.Frames.Count != 0) ??
                Enumerable.Empty<TilesetTile>();
            ...
            return tilesetTiles.Select(tile => tile.Animation!.Frames.Sum(f => f.Duration))
                .Aggregate(frameDelay, LeastCommonMultiple);
        }
        catch (Exception ex)
        {
    ...
    public async Task<(List<Image<Rgba32>> Frames, List<int> Delays)> GenerateFramesFromTilesetsAsync(
        Tilemap tilemap,
        ...
        int fps)
    {
        ...
        if (fps <= 0) throw new ArgumentException("Fps must be greater than 0.", nameof(fps));

        // Filter out any invalid tilesets
        var validTilesets = tilesets.Where(t => t.Tileset != null && t.TilesetImage != null).ToList();

        if (validTilesets.Count == 0) throw new InvalidOperationException("No valid tilesets provided.");
        ...
        try
        {
            ...
            var totalDuration = CalculateTotalDurationForTilesets(validTilesets, frameDelay);
            ...
            for (var time = 0; time < totalDuration; time += frameDelay)
            {
                ...
                if (!string.IsNullOrEmpty(tilemap.BackgroundColor))
                    ...
                    {
                        Log.Warning(ex, "Failed to parse background color: {TilemapBackgroundColor}",
                ...
                foreach (var layer in tilemap.Layers)
                {
                    ...
                    if (layerDataByName.TryGetValue(layer.Name ?? "", out var currentLayerData))
                    {
                        ...
                        await DrawTilesOnFrameWithMultipleTilesetsAsync(frame, tilemap, validTilesets, currentLayerData,
                            time);
                    }
                    else
                    {
                        ...
                        await DrawTilesOnFrameWithMultipleTilesetsAsync(frame, tilemap, validTilesets, parsedLayerData,
                            time);
                    }
                }
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                $"Error generating animation frames with multiple tilesets: {ex.Message}", ex);
        }
    }

    public int CalculateTotalDurationForTilesets(
        List<(int FirstGid, Tileset? Tileset, Image<Rgba32>? TilesetImage)> tilesets,
        int frameDelay)
    {
        ArgumentNullException.ThrowIfNull(tilesets);
        ...
        try
        {
            ...
            foreach (var (_, tileset, _) in tilesets)
            {
                ...
                if (tilesetTiles.Count > 0)
                    ...
                    animationDurations.AddRange(
                        tilesetTiles.Select(tile => tile.Animation!.Frames.Sum(f => f.Duration)));
            }

            // If no animated tiles found in any tileset, return frame delay as total duration
            if (animationDurations.Count == 0) return frameDelay;
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException(
                $"Error calculating total animation duration for multiple tilesets: {ex.Message}", ex);
        }
    }
        ...
        int currentTime)
    {
        ...
        for (var tileIndex = 0; tileIndex < layerData.Count; tileIndex++)
        {
            ...
            var mapY = tileIndex / tilemap.Width;
            ...
            if (flippedHorizontally || flippedVertically || flippedDiagonally)
                tileImage = await Task.Run(() =>
                    _tilesetImageService.ApplyTileTransformations(tileImage, flippedHorizontally, flippedVertically,
                        flippedDiagonally));
