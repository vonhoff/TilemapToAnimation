...
public class MainWorkflowTests
{
    ...
    public async Task ExecuteAsync_WithTmxInput_ProcessesCorrectly()
    {
        ...
        var frames = new List<Image<Rgba32>> { new(16, 16) };
        ...
        _tilesetServiceMock.Setup(x => x.ResolveTilesetImagePath(It.IsAny<string>(), It.IsAny<string>()))
            .Returns("test.png");
        _tilesetImageServiceMock.Setup(x => x.LoadTilesetImageAsync(It.IsAny<string>())).ReturnsAsync(tilesetImage);
        _tilesetImageServiceMock.Setup(x => x.ProcessTransparency(It.IsAny<Image<Rgba32>>(), It.IsAny<Tileset>()))
            .Returns(tilesetImage);
        _animationGeneratorServiceMock.Setup(x => x.GenerateFramesFromTilesetsAsync(
                It.IsAny<Tilemap>(),
    ...
    public async Task ExecuteAsync_WithTsxInput_NoTmxFound_ThrowsException()
    {
        ...
        try
        {
            ...
            Assert.Contains("No valid tilesets with images are available to generate animation frames",
                exception.Message);
    ...
    public async Task ExecuteAsync_WithImageInput_FindsTsxAndTmxAndProcessesCorrectly()
    {
        ...
        var tilemap = new Tilemap
        {
            ...
            Tilesets = new List<TilemapTileset>
            {
                new() { FirstGid = 1, Source = "found.tsx" }
            }
        };
